...
# 2. 建立完整模型
X <- table.b11[c("Clarity", "Aroma", "Body", "Flavor", "Oakiness", "Quality", "Region")]
fit <- lm(Quality ~ ., data = X)
print(paste("完整模型R平方:", summary(fit)$r.squared))
...
# 4. 预测和置信区间
new_data <- data.frame(
  Clarity = 1.1, Aroma = 5.1, Body = 5.6,
  Flavor = 5.5, Oakiness = 14, Region = 1
)
pred <- predict(fit_step, newdata = new_data)
ci <- qnorm(c(0.025, 0.975)) * summary(fit_step)$sigma

print(paste("预测值:", pred))
